...
In a fine example of the single biggest problem with using APIs, there have been some changes to the API since I wrote these pages in 2025. I've updated this page so it should work again and will be working through the other pages. Most noticeably, the original API URLs will no longer work in these R scripts (you'll get a 403 Forbidden error). They still work in some contexts, but I'm told the original version will go away altogether at some point. I've also spotted at least one small (but breaking) change to the JSON; there may be more. 
...
https://www.dhi.ac.uk/api-proto/view/oldbailey_record?offence=housebreaking&year_gte=1700&year_lte=1704
...
## 2026 change: new URLs

# parse the search endpoint URL into its components
query_url <-
  parse_url("https://www.dhi.ac.uk/api-proto/view/oldbailey_record")
...
single_url <- "https://www.dhi.ac.uk/api-proto/view/oldbailey_record_single"
...
## 2026 change: add value to the path

# how many hits?
search_hits <-
  search_json$hits$total$value
...
# saved search JSON for reference. updated 2026.
...
search_json$hits$hits$`_source` |>
  ...
  # change 2026: swap round first two map lines for hits.
      # previously
...
# save a copy of second page json for reference
# search_json_pages |>
#   write_json(here::here("outputs/housebreaking_page2.json"), pretty=TRUE)
```
