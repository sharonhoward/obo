...
obo_offence_category_stats_csv <-
  read_csv("https://www.dhi.ac.uk/api-proto/view/csv/agg/oldbailey_offence?series=offence_category")
...
```{r}
breaking_peace_json <- fromJSON("https://www.dhi.ac.uk/api-proto/view/oldbailey_record?offence=breakingPeace&div_type=trialAccount")
```
...
- the fixed part of the URL is **https://www.dhi.ac.uk/api-proto/view/csv/agg/**
...
NB: you may also see a `countby` in query strings when exploring searches. I don't advise using it in API queries; I tested it out quite a bit and as far as I can tell it either didn't do anything at all (!) or the `oldbailey_*` path parameter takes precedence.
...
We spent quite a while, way back in about 2008, discussing the most appropriate countby defaults for each search; if you run a few queries on the site changing the search category but keeping the default counts, you can see how it changes. 

In fact, the complexity of the statistics tool led us to write [a guide to its use](https://www.oldbaileyonline.org/about/doingstatistics).
...
obo_offence_trial_query <-
  read_csv("https://www.dhi.ac.uk/api-proto/view/csv/agg/oldbailey_trial?series=offence_category")
...
```{r}
# 2026 update: add value to path
breaking_peace_json$hits$total$value
```
...
```{r}
agg_query_url <- "https://www.dhi.ac.uk/api-proto/view/csv/agg/oldbailey_offence?rows=offence_subcategory&series=year"
...
obo_agg_variables <-
obo_agg_variables_csv |>
  mutate(url = glue("https://www.dhi.ac.uk/api-proto/view/csv/agg/oldbailey_trial?series={name}")) |>
...
obo_agg_punishment_category_csv <-
  read_csv(here::here("outputs/csv/single/punishment_category-bytrial-20260319.csv"))
